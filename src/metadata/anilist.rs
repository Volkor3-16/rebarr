...
impl ALClient {
    ...
    pub fn new() -> Self {
        // Construct ALClient
        ALClient { api_client: AniListClient::new() }
    }

    /// Performs a manga search for a given title
    /// Returns the raw Page<Vec<Media>> for external use, or Vec<Manga> for internal use.
    pub async fn search_manga(&self, title: &str) -> Result<anilist_moe::objects::responses::Page<Vec<anilist_moe::objects::media::Media>>, AniListError> {
        let response = self.api_client.manga().search_manga(title, Some(1), Some(10)).await?;
        debug!("Found {} manga results for '{}'", response.data.len(), title);
        Ok(response)
    }

    /// Returns converted Manga Vec from search results
    pub async fn search_manga_as_manga(&self, title: &str) -> Result<Vec<crate::manga::Manga>, AniListError> {
    ...
    pub async fn grab_manga(&self, id: i32) -> Result<crate::manga::Manga, AniListError> {
        ...
        debug!("Found manga '{:?}' with ID '{:?}'", response.title.as_ref().and_then(|t| t.english.as_ref()), response.id);
        Ok(response.into())
    }

    /// Grabs popular manga for new instance onboarding
    pub async fn popular_manga(&self) -> Result<Vec<crate::manga::Manga>, AniListError> {
        let page = self.api_client.manga().get_popular_manga(Some(1), Some(25)).await?;
        Ok(page.data.into_iter().map(|media| media.into()).collect())
    }
